...
def test_eu_hosts_and_uk_marketplace_contract():
    ...
    validate_payload("sp.reports.create", {"reportType": "GET_SALES_AND_TRAFFIC_REPORT", "marketplaceIds": ["A1F83G8C2ARO7P"]})
    with pytest.raises(ValueError, match="UK marketplace"):
        validate_payload("sp.reports.create", {"reportType": "x", "marketplaceIds": ["ATVPDKIKX0DER"]})
...
def test_ads_report_contract_requires_grain_columns_and_type():
    ...
    validate_payload("ads.reports.create", {
        "name": "campaign daily", "startDate": "2026-09-01", "endDate": "2026-09-02",
        "configuration": {"adProduct": "SPONSORED_PRODUCTS", "groupBy": ["campaign"],
            "columns": ["campaignId", "impressions"], "reportTypeId": "spCampaigns",
            "timeUnit": "DAILY", "format": "GZIP_JSON"},
    })


@pytest.mark.parametrize("level", [VerificationLevel.STATIC_CONTRACT, VerificationLevel.SANDBOX])
def test_non_live_evidence_never_marks_live_ready(level):
    with pytest.raises(ValueError, match="cannot mark LIVE_READY"):
        assert_live_ready_evidence(ContractEvidence("ads.keywords.update", level, "A1F83G8C2ARO7P", "123"))


def test_authorized_ads_evidence_requires_profile_scope():
    with pytest.raises(ValueError, match="profile"):
        assert_live_ready_evidence(ContractEvidence("ads.keywords.update", VerificationLevel.AUTHORIZED_LIVE_WRITE, "A1F83G8C2ARO7P"))
    assert_live_ready_evidence(ContractEvidence("ads.keywords.update", VerificationLevel.AUTHORIZED_LIVE_WRITE, "A1F83G8C2ARO7P", "123"))
