with source as (
    select * from {{ source('raw', 'raw_ads_sp_purchased_product_daily') }}
), ranked as (
...
select
    ...
    coalesce(record->>'purchasedAsin', record->>'purchased_asin', record->>'asin')::text as purchased_asin,
    coalesce(record->>'advertisedAsin', record->>'advertised_asin')::text as advertised_asin,
...
from ranked
where _rn = 1
  and coalesce(record->>'purchasedAsin', record->>'purchased_asin', record->>'asin') is not null
