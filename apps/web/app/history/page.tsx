...
function WindowPicker({ days }: { days: Window }) {
	return <div className="flex gap-1 text-xs">{ALLOWED_WINDOWS.map((d) => <Link key={d} href={windowHref("/history", d)} className={d === days ? "rounded bg-slate-900 px-2 py-1 text-white" : "rounded border border-slate-300 px-2 py-1 text-slate-600 hover:bg-slate-100 hover:text-slate-950"}>{d}d</Link>)}</div>
}
