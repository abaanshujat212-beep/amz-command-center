...
type Options = { tenants: Tenant[]; workspaces: Workspace[]; portfolioWorkspaces: Workspace[] }
...
export function ContextSwitcher({ currentTenantId, currentWorkspaceId }: { currentTenantId: string; currentWorkspaceId: string | null }) {
	...
	if (!options || (options.tenants.length <= 1 && options.workspaces.length === 0 && options.portfolioWorkspaces.length === 0)) return null
