...
import { assertBusinessHierarchy, assertWorkspaceOperation, type BusinessContext } from "@/lib/business-context"
import { assertMembership, CAN_APPROVE, type TenantRole } from "@/lib/db"

export class AuthenticationRequired extends Error { constructor() { super("Please sign in to continue.") } }
export class TenantSelectionRequired extends Error { constructor() { super("Select a tenant before opening the dashboard.") } }
...
export async function currentTenantId(): Promise<string> { return (await currentContext()).tenantId }
export async function currentUserId(): Promise<string> { return (await currentContext()).userId }
export async function canApprove(): Promise<boolean> { return CAN_APPROVE.has((await currentContext()).role) }
