...
def _to_action(row: dict) -> sm.Action:
    return sm.Action(
        ...
        entity_id=row["entity_id"], action_type=row["action_type"], before_value=row["before_value"],
        after_value=row["after_value"], status=sm.Status(row["status"]),
...
def fetch_approved(conn, tenant_id: str, limit: int) -> list[sm.Action]:
    rows = conn.execute(
        """select id, tenant_id, entity_type, entity_id, action_type, before_value,
         ...
         order by approved_at nulls first, requested_at limit %s""", (tenant_id, limit),
    ).fetchall()
...
def finish_worker_run(conn, run_id: uuid.UUID, result: WorkerResult, error: str | None = None) -> None:
    ...
    conn.execute(
        ...
        (status, result.applied, error, psycopg.types.json.Jsonb(asdict(result)), run_id),
    )
...
def load_ads_client(conn, tenant_id: str) -> AdsClient:
    row = conn.execute(
        """select c.id, c.region, c.refresh_token_encrypted, p.profile_id
         ...
         order by p.created_at nulls last limit 1""", (tenant_id,),
    ).fetchone()
    ...
    ads = AdsClient(
        ...
        tenant_id=tenant_id, region=row["region"],
    )
...
def require_current_live_consent(conn, tenant_id: str):
...
def persist_rotated_refresh_token(conn, ads: AdsClient) -> None:
    ...
    conn.execute(
        """update amazon_connection set refresh_token_encrypted = %s, key_version = %s,
               last_refresh_at = now() where id = %s""",
...
def record_apply_start(conn, action: sm.Action, *, correlation_key: str, attempt: int = 1) -> None:
    for kind, suffix in ((EventType.APPLY_REQUESTED, "requested"), (EventType.APPLY_STARTED, "started")):
        append_event(conn, event_for(
            action, kind, correlation_key=correlation_key,
            dedupe_key=f"apply:{attempt}:{correlation_key}:{suffix}", retry_attempt=attempt,
            actor_id="action-worker", previous_state=action.status.value, new_state=action.status.value,
        ))
...
def persist_apply_result(conn, action: sm.Action, api_response: dict | None = None, *, correlation_key: str | None = None, attempt: int = 1) -> None:
    conn.execute(
        """update action set status = %s, before_value = %s, applied_at = %s,
               error = %s, api_response = %s where tenant_id = %s and id = %s""",
        (action.status.value, psycopg.types.json.Jsonb(action.before_value), action.applied_at,
         action.error, psycopg.types.json.Jsonb(sanitize(api_response or {})), action.tenant_id, action.id),
    )
    ...
    append_event(conn, event_for(
        action, kind, correlation_key=correlation_key,
        dedupe_key=f"apply:{attempt}:{correlation_key}:result", retry_attempt=attempt,
        actor_id="action-worker", previous_state=sm.Status.APPROVED.value,
...
def persist_worker_alert(conn, tenant_id: str, *, kind: str, severity: str, title: str, entity_ref: str, detail: dict) -> bool:
    existing = conn.execute(
        """select id from alert where tenant_id = %s and kind = %s and entity_ref = %s
         and resolved_at is null limit 1""", (tenant_id, kind, entity_ref),
    ).fetchone()
    if existing is not None:
        return False
    conn.execute(
        "insert into alert (tenant_id, kind, severity, title, detail, entity_ref) values (%s, %s, %s, %s, %s, %s)",
...
def persist_action_failure_alert(conn, action: sm.Action) -> bool:
    ...
    return persist_worker_alert(
        conn, action.tenant_id, kind="action_failed", severity="critical",
        title=f"Action {action.id} failed: {action.action_type}", entity_ref=action.id,
        detail={"action_id": action.id, "entity_type": action.entity_type,
                "entity_id": action.entity_id, "action_type": action.action_type, "error": action.error},
    )


def persist_auth_failure_alert(conn, tenant_id: str, error: str) -> bool:
    return persist_worker_alert(
        conn, tenant_id, kind="auth_expired", severity="critical",
        title="Ads action worker could not load credentials", entity_ref="ads_api",
...
def block_live_actions(conn, actions: list[sm.Action], result: WorkerResult, *, now: dt.datetime, correlation_key: str, error: str) -> None:
    """Persist a guardrail failure without starting or calling a provider."""
    for action in actions:
        failed = sm.apply(action, now=now, live_before_value=action.before_value,
                          api_ok=False, error=f"guardrail: {error}")
...
def validate_live_batch(conn, actions: list[sm.Action], cfg, result: WorkerResult, *, now: dt.datetime, correlation_key: str) -> list[sm.Action]:
    """Validate the complete batch before any credentials are loaded."""
    eligible = []
    projected_changes = 0
    projected_budget_increase = 0.0
    with conn.cursor(row_factory=dict_row) as cur:
        for action in actions:
            try:
                validation = validate_apply_guardrails(
                    cur, action, cfg, now=now, projected_changes=projected_changes,
                    projected_budget_increase=projected_budget_increase,
                )
            except ApplyGuardrailError as exc:
                block_live_actions(conn, [action], result, now=now,
                                   correlation_key=correlation_key, error=str(exc))
                continue
            eligible.append(action)
            projected_changes += 1
            projected_budget_increase += validation.budget_increase
    return eligible


def run_once(tenant_id: str, *, limit: int = 25, client: ActionClient | None = None,
             database_url: str = DATABASE_URL, live_ads: bool = False) -> WorkerResult:
    result = WorkerResult()
    ...
    with psycopg.connect(database_url, row_factory=dict_row) as conn:
        ...
        try:
            ...
            if live_ads:
                try:
                    cfg = require_current_live_consent(conn, tenant_id)
                except GuardrailConfigError as exc:
                    block_live_actions(conn, actions, result, now=now,
                                       correlation_key=str(run_id), error=str(exc))
                    finish_worker_run(conn, run_id, result, error=str(exc))
                    conn.commit()
                    return result
                actions = validate_live_batch(
                    conn, actions, cfg, result, now=now, correlation_key=str(run_id)
                )
                if not actions:
                    finish_worker_run(conn, run_id, result, error="all actions blocked by guardrails")
                    conn.commit()
                    return result
            if client is None:
                if live_ads:
